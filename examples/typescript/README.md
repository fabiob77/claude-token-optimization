...
| File | What it shows |
|---|---|
| `src/01_basic_caching.ts` | Minimal prompt caching, prints usage so you can watch the 90% discount kick in on the second call. |
...
- Caching has a **1,024-token minimum** on current models. The examples use prompts long enough to qualify; if you swap in shorter content the cache silently misses.
- The Batch API only takes **non-streaming** requests, and the official SLA is 24 hours (most batches finish in under an hour).
- Model strings (`claude-opus-4-7`, `claude-sonnet-4-6`, `claude-haiku-4-5`) are valid as of April 2026. Anthropic also publishes dated aliases; check https://platform.claude.com for the latest.
